...
/* [DHRUT-V]: results stashed here for post-run inspection (no UART to
 * print through) - see tools/bench_report.py / dhrystone_main.c bottom. */
long dhrutv_final_dmips;
long dhrutv_final_dmips_per_mhz_num;
long dhrutv_final_mhz;
long dhrutv_final_cycles;
long dhrutv_final_runs;
  ...
  /* Main and Proc_0 in the Ada version             */
{
  ...
  /* [DHRUT-V]: self-check against the well-known Dhrystone 2.2 final
   * values (see the "should be" debug_printf lines above, which are
   * silent here since there's no UART) instead of relying on eyeballing
   * printed output. Also stash the perf numbers in globals that a
   * debugger/cpu_trace.log can inspect post-run. Returns 0 iff every
   * check passes; each failing check sets a distinct bit so a nonzero
   * exit code (visible via tohost) pinpoints which one broke. */
  dhrutv_final_dmips        = Dhrystone_MIPS;
  dhrutv_final_dmips_per_mhz_num = Dhrystone_MIPS;
  dhrutv_final_mhz           = MHZ;
  dhrutv_final_cycles        = User_Time;
  dhrutv_final_runs          = Number_Of_Runs;

  {
    ...
    if (Ptr_Glob->Discr != 0)                                  fail |= (1 << 5);
    if (Ptr_Glob->variant.var_1.Enum_Comp != 2)                fail |= (1 << 6);
    if (Ptr_Glob->variant.var_1.Int_Comp != 17)                fail |= (1 << 7);
    if (Next_Ptr_Glob->Discr != 0)                             fail |= (1 << 8);
    if (Next_Ptr_Glob->variant.var_1.Enum_Comp != 1)           fail |= (1 << 9);
    if (Next_Ptr_Glob->variant.var_1.Int_Comp != 18)           fail |= (1 << 10);
    if (Int_1_Loc != 5)                                        fail |= (1 << 11);
    if (Int_2_Loc != 13)                                       fail |= (1 << 12);
