...
module decoder (
  ...
  input  logic        i_pred_taken,    // prediction made for this PC at fetch
  ...
  // ---------------------------------------------------------------------------
  assign opcode  = riscv_opcode_t'(i_instr[6:0]);
  ...
  // ---------------------------------------------------------------------------
  // Immediate generation (RV32I standard)
  // ---------------------------------------------------------------------------
  ...
  assign imm_j = {{11{i_instr[31]}}, i_instr[31], i_instr[19:12], i_instr[20], i_instr[30:21], 1'b0};
  ...
  always_comb begin
    ...
    if (i_valid) begin
      ...
      o_uop.rs2          = rs2;
      o_uop.rd           = rd;
      o_uop.pred_taken   = i_pred_taken;
      o_uop.pred_target  = i_pred_target;
      o_uop.instr_bits   = i_instr;
      ...
      case (opcode)
        ...
        OPCODE_SYSTEM: begin
          ...
          o_uop.imm          = {20'b0, i_instr[31:20]};  // CSR addr (funct3!=0) or funct12 (funct3==0)
